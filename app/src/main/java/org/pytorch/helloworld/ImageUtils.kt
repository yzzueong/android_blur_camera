...
abstract class ImageUtils {
    companion object {
        ...
        fun scaleBitmapAndKeepRatio(
             ...
             */
            val resizeRatio: Float = (reqHeightInPixels.toFloat() / Math.max(targetBmp.width, targetBmp.height))
            val rw : Int = Math.round(targetBmp.width * resizeRatio)
            val rh : Int = Math.round(targetBmp.height * resizeRatio)

            val temp = Bitmap.createBitmap(rw, rh, Bitmap.Config.ARGB_8888)
            val canvas = Canvas(temp)
            canvas.drawBitmap(targetBmp,
                    Rect(0, 0, targetBmp.width, targetBmp.height),
                    Rect(0, 0, rw, rh),
                    null)
            var resized = extendBitmap(temp, reqWidthInPixels,reqHeightInPixels, Color.BLACK)
            return resized
        }
        ...
        fun extendBitmap(origin: Bitmap, destW: Int, destH: Int, backgroundColor: Int): Bitmap {
            if (origin != null && destW > 0 && destH > 0) {
                val var4 = origin.width
                val var5 = origin.height
                if (destW >= var4 && destH >= var5) {
                    val var6 : Bitmap = Bitmap.createBitmap(destW, destH, Bitmap.Config.ARGB_8888)
                    val var7 = Canvas(var6)
                    val var8 = Paint(1)
                    var7.drawColor(backgroundColor)
                    val var9 : Int = Math.round((destW - var4).toDouble() / 2.0).toInt()
                    val var10 : Int = Math.round((destH - var5).toDouble() / 2.0).toInt()
                    var7.drawBitmap(origin, var9.toFloat(), var10.toFloat(), var8)
                    return var6
                } else {
                    return origin
                }
